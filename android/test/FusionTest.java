 ...
 * Axis mapping (regression-validated on recorded replay): rot=0 dx=-wx*dt*S,
 * dy=+wy*dt*S; rot=180 negates both.
 */
public class FusionTest {
    ...
    public static void main(String[] args) {
        ...
        //    wx=0.05, wy=0.1 rad/s for 1s -> dx=-50, dy=+100 at rot=0
        Fusion f = new Fusion();
        f.setScale(1000f);
        f.setRotation(0);
        ...
        check(Math.abs(s.x - 910f) < 0.5f, "dx = -wx*dt*S = -50 at rot=0");
        check(Math.abs(s.y - 640f) < 0.5f, "dy = +wy*dt*S = +100 at rot=0");
        ...
        f.onGyro(NS, 0, 0, 0);
        feedGyro(f, NS, (long) (1.2 * NS), 2, 0f, 0.1f); // p.y -> 620
        long t = (long) (1.2 * NS);
        for (int i = 0; i < 30; i++) {
        ...
        s = f.snapshot(t);
        System.out.println("convergence: x=" + s.x + " y=" + s.y);
        ...
        //    feed wx=0.1 -> dx=-10px/tick
        f = new Fusion();
        f.setScale(1000f);
        f.onCameraLock(2 * NS, 960, 540);
        f.onGyro(2 * NS, 0, 0, 0);
        f.onCameraUnlock(2_050_000_000L);
        feedGyro(f, 2 * NS, (long) (3.5 * NS), 15, 0.1f, 0f); // 0.1s ticks up to t=3.5s
        s = f.snapshot(3 * NS);
        System.out.println("predict window: x=" + s.x + " valid=" + s.valid + " pred=" + s.predicted);
        check(Math.abs(s.x - 860f) < 0.5f, "only ticks <=1s after last lock integrate (10 ticks = -100px)");
        ...
        check(Math.abs(s.x - 860f) < 0.5f, "state frozen after window (no drift)");
        ...
        check(Math.abs(s.x - 0.7f * 860f - 0.3f * 960f) < 0.5f,
        ...
        check(Math.abs(s.x - 1010f) < 0.5f, "rot=180 negates dx (-wx -> +50)");
        check(Math.abs(s.y - 440f) < 0.5f, "rot=180 negates dy (+wy -> -100)");
        ...
        feedGyro(f, 2_100_000_000L, 2_200_000_000L, 1, 0f, 0.1f); // dt=0.1s, dy=+10
        s = f.snapshot(2_200_000_000L);
        check(Math.abs(s.y - 410f) < 0.5f, "gyro dt seamless across first lock (clock pre-accumulated)");
        ...
        check(s.y == 540f, "gyro gap > MAX_DT is clamped (no jump)");
        f.onGyro(NS + 550_000_000L, 0, 10f, 0); // 0.05s: integrates, dy=+500
        s = f.snapshot(NS + 550_000_000L);
        check(Math.abs(s.y - 540f - 0.05f * 10f * 1000f) < 0.5f, "integration resumes after gap");
