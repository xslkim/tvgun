...
|---|---|---|
| 可用率（有输出） | 48.2% / 36.8%（仅 FULL 锁定） | **99.2% / 98.0%**（含 PARTIAL/EDGE/GYRO） |
| 静止抖动 | 0.38 规范 px | 0.38 / 0.55 规范 px |
| FULL 精度（vs 参考，持续段） | — | 中位 0.98 / 广角稳态系统偏 +6.9px（待校准吸收） |
| 运动段滞后（vs 无滞后真值） | 5.0px@100-200px/s、38.1px@>500px/s | **1.8px、15.6px**（2.4-2.8× 改善） |
| 再锁定校正量（失锁回归时） | 中位 105-170px | **中位 6.2-11.7px，P95 19-26px** |
| 伪失锁外推（MC 挖窗 1s） | 中位 ~111px | **中位 37px（主摄）/ 41px（广角）** |
| aim 上报率 | ≤15Hz（绑相机帧） | **60Hz**（独立线程，陀螺速率快照） |
| Java/Python 回放等价 | — | 主摄 grade 一致 86.7%（混沌边界），广角 99.9%；co-FULL 准星差 0.27/0.36px；可用帧数完全一致 |
| Java 耗时 | — | 1.3ms/帧（桌面 JVM；手机大核 ~4-6ms，预算充足） |
...
| `record_wide_20260925_214838/` | 超广角 92°（c2:21），~15s/422帧，真机实测录制 |
